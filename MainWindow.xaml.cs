...
public partial class MainWindow : Window
{
    ...
    private void UpdateServerRam()
    {
        ...
        if (proc != null && !proc.HasExited)
        {
            try
            {
                ...
                long ramMB = proc.PrivateMemorySize64 / (1024 * 1024);
    ...
    private void LoadSettings(bool silent = false, bool isReloading = false)
    {
        try
        {
            ...
            // 5. Устанавливаем флаг isConfigured в зависимости от валидности путей
    ...
    private async void StartServerAsync()
    {
        ...
        if (_authFailedHandled)
        {
            ...
            Log("Please fix the 'authentication token' in the configuration and restart the manager.", "WARN");
            return;
        }
    ...
    private async Task StopServer(Process process)
    {
        ...
        // Асинхронное ожидание завершения с таймаутом (10 секунд)
        ...
        Dispatcher.Invoke(() =>
        {
            ...
            TxtServerStatus.Visibility = Visibility.Visible;
            BackupProgress.IsIndeterminate = false;
            BackupProgress.Visibility = Visibility.Collapsed; // скрыть прогресс-бар, когда сервер не работает
            StatusBarText.Text = "Ready";
            UpdateServerInfo();
        });
    }
